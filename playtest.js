...
(async () => {
  ...
  // 2. Wait for React to mount (skeleton or app)
  await page.waitForTimeout(3500);

  // 3. Check root is not empty
  ...
  const initErrors = consoleErrors.filter(e => !e.includes('in-browser Babel') && !e.includes('babel'));
  ...
  // 6. Press Cmd+K to open palette (keyboard shortcut)
  ...
  // 7. Rapid repeated clicks on Run Batch
  ...
  // 8. Open palette, type nonsense, confirm 'No matches' empty state
  ...
  // 9. Type a real command and execute
  ...
  // 10. Navigate tabs via nav clicks
  const ledgerTab = page.locator('.nav-item:has-text("Case Ledger")');
  ...
  const themeBefore = await themeBtn.getAttribute('aria-label');
  ...
  // 16. Audit modal flow: pick a case if ledger has rows
